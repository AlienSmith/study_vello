...
impl Render {
    ...
    pub fn render_encoding_coarse(
        ...
        let path_info_buf =
            ResourceProxy::new_buf(buffer_sizes.path_info.size_in_bytes().into(), "path_info");
        ...
        if wg_counts.use_patterns || wg_counts.use_instance {
            recording.dispatch(
                shaders.pattern_setup,
                (1, 1, 1),
                [config_buf, bump_buf, indirect_count_buf.into()],
            );
        ...
        let mut run_coarse = || {
            recording.dispatch(
                shaders.coarse_counter,
                wg_counts.coarse_counter,
                [
                    config_buf,
                    scene_buf,
                    draw_monoid_buf,
            ...
            recording.dispatch(
                shaders.fine_setup,
                (1, 1, 1),
                [config_buf, indirect_count_buf.into(), bump_buf],
            );
        };
