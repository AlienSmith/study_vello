            ...
            match event {
                    ...
                    match event {
                                            ...
                                            "g" => {
                                                        ...
                                                        &mut renderers[render_state.surface.dev_id]
                                                {
                                                    renderer.profiler
                                                        ...
                                                        .expect(
                                                            "Not setting max_num_pending_frames"
                                                        );
                                                }
                                            }
                                            "v" => {
                        ...
                        WindowEvent::Touch(touch) => {
                            match touch.phase {
                                TouchPhase::Started => {
                                    ...
                                    if
                                        touch.location.y >
                                        ((render_state.surface.config.height as f64) * 2.0) / 3.0
                                    {
                                        navigation_fingers.insert(touch.id);
                                        // The left third of the navigation zone navigates backwards
                                        if
                                            touch.location.x <
                                            (render_state.surface.config.width as f64) / 3.0
                                        {
                                            scene_ix = scene_ix.saturating_sub(1);
                                        } else if
                                            touch.location.x >
                                            (2.0 * (render_state.surface.config.width as f64)) / 3.0
                                        {
                                            scene_ix = scene_ix.saturating_add(1);
                                        }
                                    }
                        ...
                        WindowEvent::RedrawRequested => {
                            ...
                            if prev_scene_ix != scene_ix {
                                ...
                                render_state.window.set_title(
                                    &format!("Vello demo - {}", example_scene.config.name)
                                );
                            }
                            fragment.reset();
                            ...
                            let mut base_color = args.args.base_color
                                .or(scene_params.base_color)
                                .unwrap_or(vello::peniko::Color::TRANSPARENT);
                            if base_color != vello::peniko::Color::TRANSPARENT {
                                println!(
                                    "at this point ptcl segmentation does not support none transparent base color"
                                );
                            }
                            ...
                            if let Some(resolution) = scene_params.resolution {
                                ...
                                let scale_factor = (factor.x / resolution.x).min(
                                    factor.y / resolution.y
                                );
                            ...
                            if stats_shown {
                                ...
                                #[cfg(feature = "wgpu-profiler")]
                                if
                                    let Some(profiling_result) = renderers[
                            ...
                            let surface_texture = render_state.surface.surface
                                .get_current_texture()
                                .expect("failed to get surface texture");
                            #[cfg(not(target_arch = "wasm32"))]
                            {
                                scene_complexity = vello
                                    ::block_on_wgpu(
                ...
                Event::AboutToWait => {
                    ...
                    if let Some(touch_info) = touch_info {
                        ...
                        transform =
                            Affine::translate(touch_info.translation_delta) *
                ...
                Event::Resumed => {
                    ...
                    #[cfg(not(target_arch = "wasm32"))]
                    {
                        let Option::None = render_state else {
                            return;
                        };
                        ...
                        let surface = pollster
                            ::block_on(surface_future)
                        ...
                        render_state = {
                            ...
                            renderers[id].get_or_insert_with(|| {
                                ...
                                Renderer::new(
                                    ...
                                    &(RendererOptions {
                                        surface_format: Some(render_state.surface.format),
                                        timestamp_period: render_cx.devices[
                                            id
